...
# REQUIRED_AP_COLUMNS = ['Company Code', 'Supplier', 'Amount', 'Reference']


# def validate_ap_registry(df):
#     missing = [c for c in REQUIRED_AP_COLUMNS if c not in df.columns]
#     if missing:
#         raise RegistryMismatchError(
#             "This file doesn't look like an AP registry. "
#             f"Missing expected column(s): {', '.join(missing)}. "
#             "Make sure you selected the right file for the Accounts Payable process."
#         )
...
#     # Fail loudly if this isn't actually an AP registry, instead of
#     # silently writing blank/garbage rows and returning a "success".
#     validate_ap_registry(df_raw)
...
def read_ap_registry(registry_file):
    """
    Read the AP registry with the correct header row, and confirm it's an
    AP registry at all.

    Different exports put the header in different places: some have
    human-readable labels on Row 1 with the technical SAP codes as a
    stray Row 2 ('Company Code' / 'BUKRS'), others have it the other way
    around (technical codes on Row 1, labels on Row 2). Every field lookup
    in this module reads by the human-readable label
    (row_data.get("Company Code"), etc.), so picking the wrong row as the
    header silently produces an all-blank output instead of an error.

    This checks both of the first two rows for the required column names
    and uses whichever one actually contains them as the header.
    """
    preview = pd.read_excel(registry_file, header=None, nrows=2)
    registry_file.seek(0)

    def row_values(row):
        return {str(v).strip() for v in row if pd.notna(v)}

    row0_vals = row_values(preview.iloc[0]) if len(preview) > 0 else set()
    row1_vals = row_values(preview.iloc[1]) if len(preview) > 1 else set()

    row0_matches = sum(1 for c in REQUIRED_AP_COLUMNS if c in row0_vals)
    row1_matches = sum(1 for c in REQUIRED_AP_COLUMNS if c in row1_vals)

    if row0_matches == 0 and row1_matches == 0:
        raise RegistryMismatchError(
            "This file doesn't look like an AP registry. "
            f"None of the expected column(s) ({', '.join(REQUIRED_AP_COLUMNS)}) "
            "were found in the first two rows. "
            "Make sure you selected the right file for the Accounts Payable process."
        )

    if row1_matches > row0_matches:
        # Row 2 holds the human-readable labels — use it as the header.
        # Row 1 (the technical-code row) is then naturally excluded, since
        # pandas only reads data starting after the header row.
        df = pd.read_excel(registry_file, header=1)
    else:
