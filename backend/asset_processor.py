...
# #         # Rows on THIS sheet that have at least one blank mandatory field.
# #         # Filled in red once we know all rows have been written.
...
# #             if row_has_missing_mandatory:
# #                 rows_with_missing_mandatory.add(current_row)

# #             current_row += 1

# #         # Highlight every flagged row across all of its used columns so
# #         # the missing-mandatory-field rows are obvious at a glance.
...
#     # df_raw = pd.read_excel(registry_file)
#     try:
#         df_raw = pd.read_excel(registry_file, sheet_name="CA01")
#     except ValueError:
#         raise RegistryMismatchError(
#             "The uploaded Excel file does not contain a sheet named 'US01'. "
#             "Please ensure you are uploading a valid Asset registry file."
#         )
...
#         # A genuinely empty cell (pandas gives NaN/NaT for this) means
#         # the asset was never deactivated, i.e. it's active. Some ECC
#         # exports instead write a literal placeholder date for "no
#         # deactivation" — handle both.
...
#         # Highlight rows that have at least one blank mandatory field
#         last_col = len(tech_cols) if tech_cols else ws.max_column
#         for flagged_row in rows_with_missing_mandatory:
#             for c_idx in range(1, last_col + 1):
#                 ws.cell(row=flagged_row, column=c_idx).fill = MISSING_MANDATORY_FILL
...
def process_asset_registry(
    ...
    sheet_names=None          # NEW: list of sheet names to process, e.g. ['US01','US06','CA01']
) -> io.BytesIO:
    """
    Processes the ECC asset registry spreadsheet(s), filters active records,
    maps fields to S/4 HANA, and writes them to the prebuilt Excel template sheets starting at Row 8.

    :param registry_file: file-like object or path to the Excel file
    :param template_path: path to the output template
    :param custom_mappings: dict containing:
        - 'cocd': dict
        - 'plant_loc': dict with key: (plant_val, location_str) and value: {"s4_plant": str, "s4_location": str}
        - 'cost_center': dict
    :param sheet_names: list of sheet names to read and combine, e.g. ['US01','US06','CA01']
                        If None, defaults to ['CA01'] for backward compatibility.
    :return: (io.BytesIO, list) – the generated workbook and a list of validation errors.
    """
    if custom_mappings is None:
        custom_mappings = {}

    # If no sheet_names provided, use the old default
    ...
    # 1. Read all specified sheets and combine them
    dfs = []
    missing_sheets = []
    for sheet in sheet_names:
        try:
            df_sheet = pd.read_excel(registry_file, sheet_name=sheet)
            # Optionally add a column to remember the source sheet (uncomment if needed)
    ...
    if missing_sheets:
        raise RegistryMismatchError(
            f"The uploaded Excel file does not contain the following sheet(s): {', '.join(missing_sheets)}. "
            "Please ensure you are uploading a valid Asset registry file with all required sheets."
        )
